...
(function() {
  var BaseModel, Email, flashError, loadPage, mergeEmails, serializeKey, toggleLoading,
  ...
  window.search = function(form) {
    ...
    query = (_ref = sjcl.searchable).createQuery.apply(_ref, [window.searchKeys].concat(__slice.call(tokens)));
    return form.search.value = JSON.stringify(query);
  };
  ...
  window.page = function(i) {
    ...
    return window.socket.emit('page', i, window.query);
  };
  ...
  window.delEmail = function(id) {
    ...
    if (sure) {
      ...
      return window.socket.once('delete', function(minused, deleted) {
        var dn, _i, _j, _len, _len1;
        for (_i = 0, _len = minused.length; _i < _len; _i++) {
          dn = minused[_i];
          window.searchKeys[dn][0]--;
        }
        for (_j = 0, _len1 = deleted.length; _j < _len1; _j++) {
          dn = deleted[_j];
          delete window.searchKeys[dn];
        }
        window.socket.emit('updateKey', serializeKey());
        return window.socket.once('updateKey', function() {
          return window.location = '/inbox';
        });
      });
    }
  ...
  loadPage = function(p, emails, pages) {
    var c, html, i, last, li, loadEmail;
    ...
    if (emails.length !== 0) {
      ...
      if (p !== 1) {
        html = '<li><a href="#" onclick="window.page(' + (p - 1) + ')">&laquo;</a></li>';
      } else {
      ...
      if (p !== pages) {
        last = '<li><a href="#" onclick="window.page(' + (p + 1) + ')">&raquo;</a></li>';
      } else {
  ...
  $(document).ready(function() {
    ...
    if (window.privKey != null) {
      ...
      window.socket.on('authed', function() {
        ...
        pages = window.pages;
        return loadPage(1, emails, pages);
      });
    }
    if (window.localStorage.query != null) {
    ...
    return window.socket.on('index', function(newIds, newDomain, reps, emails) {
      var dn, key, out, _i, _len, _ref;
